...
export const protobufPackage = "mycoin.mycoin";
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("mycoin.mycoin.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  SayHello(request: QuerySayHelloRequest): Promise<QuerySayHelloResponse> {
    const data = QuerySayHelloRequest.encode(request).finish();
    const promise = this.rpc.request("mycoin.mycoin.Query", "SayHello", data);
